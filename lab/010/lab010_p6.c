...
// Employee code , Employee name, Salary, Department Name, Date of joining (it is itself a
// structure consisting of day, month, and year)
...
// Use a function to display the employee information while getting the maximum and
// minimum salary.
...
//  Use a function to display the employee records in ascending order according to their
// salary.
...
void displayEmployeeRecordsByJoiningDate(struct Employee *employees, int n)
{
    for (int i = 0; i < n; i++)
    {
        for (int j = i + 1; j < n; j++)
        {
            if (employees[i].date_of_joining.year > employees[j].date_of_joining.year)
            {
                struct Employee temp = employees[i];
                employees[i] = employees[j];
                employees[j] = temp;
            }
            else if (employees[i].date_of_joining.year == employees[j].date_of_joining.year)
            {
                if (employees[i].date_of_joining.month > employees[j].date_of_joining.month)
                {
                    struct Employee temp = employees[i];
                    employees[i] = employees[j];
                    employees[j] = temp;
                }
                else if (employees[i].date_of_joining.month == employees[j].date_of_joining.month)
                {
                    if (employees[i].date_of_joining.day > employees[j].date_of_joining.day)
                    {
                        struct Employee temp = employees[i];
                        employees[i] = employees[j];
                        employees[j] = temp;
...
int main()
{
    ...
    {
        printf("\n\n\n");
        printf("1. Display the employee information while getting the maximum and minimum salary.\n");
        printf("2. Display the employee records in ascending order according to their salary.\n");
        printf("3. Display the employee records in ascending order according to their date of joining.\n");
